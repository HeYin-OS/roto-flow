...
class MainWindow(QMainWindow):
    ...
    def onPredictCurves(self):
        ...
        for idx_frame in range(idx_start, idx_end):
            for curve_real in self.canvas.curves_on_each_frame[idx_frame]:
                ...
                for point_ready in curve_real:
                    ...
                    x_predict = x_real + self.canvas.test_video.optical_flow_cache[idx_frame, x_real, y_real, 0]
                    y_predict = y_real + self.canvas.test_video.optical_flow_cache[idx_frame, x_real, y_real, 1]

                    curve_predict.append(QPoint(int(x_predict), int(y_predict)))

                # TODO: edge snapping "curve_predict" here

                self.canvas.curves_on_each_frame[idx_frame + 1].append(curve_predict)
